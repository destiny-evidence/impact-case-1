...
AND
(
  (health OR wellbeing OR "well-being" OR ill OR illness OR disease* OR syndrome* OR infect* OR medical* OR mortality OR
  DALY OR morbidity OR injur* OR death* OR hospital* OR acciden* OR emergency OR emergent OR doctor OR GP OR obes* OR
   overweight OR "over-weight" OR underweight OR "under-weight" OR hunger OR stunting OR wasting OR undernourish* OR
   undernutrition OR anthropometr* OR malnutrition OR malnour* OR anemia OR anaemia OR "micro-nutrient*" OR hypertension OR
   "blood pressure" OR stroke OR renovascular OR cardiovascular OR cerebrovascular OR (CVD NOT (vapor OR vapour)) OR
   "heart disease" OR Isch*emic OR cardio*vascular OR "heart attack" OR "heart attacks" OR coronary OR CHD OR diabet* OR
   CKD OR renal OR cancer OR kidney OR lithogenes* OR skin OR fever* OR renal* OR rash* OR eczema* OR "thermal stress" OR
   hypertherm* OR hypotherm* OR pre*term OR stillbirth OR birth*weight OR LBW OR maternal OR pregnan* OR gestation* OR
   "pre-eclampsia" OR "preeclampsia" OR sepsis OR oligohydramnios OR placenta* OR haemorrhage OR hemorrhage OR malaria OR
    dengue* OR mosquito* OR chikungunya OR leishmaniasis OR encephalit* OR vector-borne OR pathogen OR zoonos* OR zika* OR
    "west nile" OR onchocerciasis OR filiariasis OR waterborne OR diarrhoeal OR diarrheal OR gastro* OR (enteric NOT
     (fermentation OR "enteric CH4" OR "enteric methane")) OR "vibrio bacteria" OR cyanobacteria OR parasit* OR borrelia
      OR paraly* OR neurotoxi* OR viral OR rotavirus OR noravirus OR hantavirus OR cholera OR protozoa* OR lyme OR tick*borne
