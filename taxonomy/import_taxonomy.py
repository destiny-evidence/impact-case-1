...
def parse_vocabulary_to_nacsos(ttl_filepath: str, project_id: str, scheme_id: str):
    ...
    def build_choices(concept_list: list, label_key: str, depth: int,
                      scheme_uri: str, scheme_name: str) -> list[AnnotationSchemeLabelChoice]:
        choices_output = []
    ...
    for scheme in sorted(schemes, key=lambda s: s["name"].casefold()):
        ...
        nacsos_labels.append(AnnotationSchemeLabel(
            name=scheme["name"],
            key=scheme_key,
    ...
    scheme_description = (
        f"Generated from the DESTINY SKOS vocabulary ({fname}): "
        f"{len(nacsos_labels)} dimensions, {len(mapping)} concepts."
    )
