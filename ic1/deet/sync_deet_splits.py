...
def main(task: Annotated[TaskName, typer.Option(help='The annotation task task to be exported')] = TaskName.ALL):
    ...
    for task_config in selected.values():
        ...
        validation_ids = [internal_external_id_map[iid] for iid in deet_splits.development_ids + deet_splits.validation_ids]
        test_ids = [internal_external_id_map[iid] for iid in deet_splits.test_ids]
        splits.add_items(validation_ids=validation_ids, test_ids=test_ids, exclude_deet=True)
