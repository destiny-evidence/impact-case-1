...
def find_best_model(path: Path) -> ModelRun:
    ...
    runs = [ModelRun.model_validate_json(l) for l in path.read_text().splitlines() if l.strip()]
    if not runs:
...
def main(dev_mode: bool = True):
    ...
    INOUT.ml_test_result_path.write_text(test_result.model_dump_json(indent=2))

    clf.save(INOUT.ml_model_path)

    predictions = pd.DataFrame(
        {'item_id': test['item_id'].to_list(), 'score': y_pred_proba, 'predicted_label': (y_pred_proba >= best_model.threshold).astype(int)}
    )
